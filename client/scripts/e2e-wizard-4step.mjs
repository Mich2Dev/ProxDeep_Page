...
try {
  ...
  await pickButton(page, 'Procesamiento de Documentos e Información No Estructurada');
  await page.waitForTimeout(500);
  await pickButton(page, 'Datos Listos para Producción');
  await page.waitForTimeout(500);
  ...
  ok('Foco anterior limpiado', !(await page.getByText('Foco: NLP & Documentos').isVisible().catch(() => false)));

  console.log('\n=== E2E 3: Rama B + conflicto + submit demo ===');
  await pickButton(page, 'SaaS y Nube Comercial');
  await page.waitForTimeout(500);
  await pickButton(page, 'Datos Centralizados sin Limpieza Operativa');
  await page.waitForTimeout(500);
  ...
  await pickButton(page, 'Analítica Predictiva y Modelado de Datos Estructurados');
  await page.waitForTimeout(400);
  await pickButton(page, 'Datos Crudos y Dispersos');
  await page.waitForTimeout(400);
  await page.getByRole('button', { name: /Infraestructura: Creación desde cero/i }).click();
  await page.waitForTimeout(400);
  ok('Reopen paso 1 activo', await page.getByText('Paso 1 · Activo').isVisible());
  ok('Paso 2 limpiado tras reopen paso 1', !(await page.getByText('Foco: Data Analytics').isVisible().catch(() => false)));
